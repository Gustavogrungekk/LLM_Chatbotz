...
class AthenaTool():
    ...
    def get_latest_date(self):
        # Retorna a última data disponível (placeholder)
...
class MrAgent():
    def __init__(self):
        ...
        self.athenas_time = []  # Para armazenar tempos de execução das queries no Athena
        self.init_model()
        self.build_workflow()
    
    # ========== NOVO PROMPT PARA GERAÇÃO DE QUERIES ==========
    sql_gen_prompt_str = """
    ...
    **Instruções:**
    ...
    def init_model(self):
        ...
        self.date_prompt = ChatPromptTemplate.from_messages(
            ...
            - Se não houver data, usar {latest_date}
        ...
        # Configuração dos modelos para enriquecimento e resposta, vinculando ferramentas se necessário
        self.model_enrich_mr_camp = self.suges_pergunta_prompt | ChatOpenAI(model="gpt-4-0125-preview", temperature=0, seed=1)
        self.model_mr_camp = ChatOpenAI(model="gpt-4-0125-preview", temperature=0, seed=1).bind_tools(
    ...
    def call_tool(self, state):
        # Gera a query SQL utilizando o modelo configurado
