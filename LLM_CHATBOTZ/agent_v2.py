...
class InsightsAgent:
    def __init__(self, llm, prompt: str):
        self.prompt = prompt  # Pode ser configurado via arquivo YAML (ex: config/prompts.yaml)
        self.llm = llm
    
    def generate(self, df: pd.DataFrame) -> str:
        data_str = df.to_csv(index=False)
        prompt_with_data = (
            "Você tem acesso ao dataframe retornado da consulta SQL, apresentado em formato CSV abaixo:\n\n"
            "{data}\n\n"
            "Com base nesses dados, extraia os principais insights sobre o negócio, identifique tendências, "
            "pontos de atenção e oportunidades, e forneça sugestões de ações que podem ser implementadas para melhorar os resultados. "
            "Apresente as informações de forma clara e organizada, de maneira que possam ser facilmente compreendidas em um contexto de chat."
        ).format(data=data_str)
        return self.llm.invoke({"data": prompt_with_data})
...
class AgentState(TypedDict):
    ...
    response: str
    error: str
    df: pd.DataFrame  # incluímos o dataframe no estado
